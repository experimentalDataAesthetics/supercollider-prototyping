// Source: Chapter 16 Microsound by Alberto de Campo in "The SuperCollider Book" (MIT 2011).
...
Pbindef(\lo,
	...
	\freq, 1000, \amp, 0.2, \dur, 0.5, \lag, 0
).play;
Pbindef(\hi,
	\instrument, \percSin, \sustain, 0.05,
	\freq, 1000, \amp, 0.1, \dur, 0.5, \lag, 0
).play;
)
	// try different lag times between them
Pbindef(\hi, \lag, 0.2); // 200ms
Pbindef(\hi, \lag, 0.1); // 100ms
Pbindef(\hi, \lag, 0.08);

Pbindef(\hi, \lag, 0.05); // "sensitive zone" (sweet spot) is somewhere here
Pbindef(\hi, \lag, 0.45); // "sensitive zone" (sweet spot) is somewhere here
Pbindef(\hi, \lag, 0.035);// "sensitive zone" (sweet spot) is somewhere here
Pbindef(\hi, \lag, 0.03); // "sensitive zone" (sweet spot) is somewhere here
Pbindef(\hi, \lag, 0.025);
Pbindef(\hi, \lag, 0.02);
Pbindef(\hi, \lag, 0.01);
Pbindef(\hi, \lag, 0.0105);
Pbindef(\hi, \lag, 0.015);
Pbindef(\hi, \lag, 0.009);


// Now with a lower frequency of the percussive sound
...
Pbindef(\hi, \lag, 0.04);
Pbindef(\hi, \lag, 0.035); // "sensitive zone" (sweet spot) is somewhere here
Pbindef(\hi, \lag, 0.03); // 30ms
Pbindef(\hi, \lag, 0.02); //
Pbindef(\hi, \lag, 0.01); // strange non-linearity?
Pbindef(\hi, \lag, 0.0105);
Pbindef(\hi, \lag, 0.015);
Pbindef(\hi, \lag, 0.009);

Pbindef(\hi, \lag, 0.007);
...
~silence.spawn([\silDur, 0.3]);	  // a pause in the sound
...
Pbindef(\grain4,
	...
	\freq, Pshuf([1000, 600, 350, 250, 5, 2000]), // random every each time
	\dur, 0.005
).play;
